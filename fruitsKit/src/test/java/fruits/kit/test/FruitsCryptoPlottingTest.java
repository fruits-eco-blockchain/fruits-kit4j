...
public class FruitsCryptoPlottingTest {
    ...
    private void testPlot(byte pocVersion) {
        ...
        // Reference plotter
        new MiningPlot(fruitsCrypto.getShabal256(), testAccountId, testNonce, pocVersion, reference, 0);

        byte[] compare = new byte[MiningPlot.PLOT_SIZE];
        fruitsCrypto.plotNonce(testAccountId, testNonce, pocVersion, compare, 0);
    ...
    private void testPlotMultiple_smallCount(byte pocVersion) {
        ...
        // Reference plotter
        new MiningPlot(fruitsCrypto.getShabal256(), testAccountId, testNonce, pocVersion, firstNonceReference, 0);
        new MiningPlot(fruitsCrypto.getShabal256(), testAccountId, testNonce + 1, pocVersion, secondNonceReference, 0);
        ...
        fruitsCrypto.plotNonces(testAccountId, testNonce, 2, pocVersion, bothNonces, 0);
    ...
    void testPlotMultiple_largeCount(byte pocVersion) {
        ...
        for (int i = 0; i < nonceCount; i++) {
            new MiningPlot(fruitsCrypto.getShabal256(), testAccountId, testNonce + i, pocVersion, reference, i * MiningPlot.PLOT_SIZE);
        }

        byte[] compare = new byte[nonceCount * MiningPlot.PLOT_SIZE];
        fruitsCrypto.plotNonces(testAccountId, testNonce, nonceCount, pocVersion, compare, 0);
