/*
  Reed Solomon Encoding and Decoding for Fruits

  Version: 1.0, license: Public Domain, coder: NxtChg (admin@nxtchg.com)
  Java Version: ChuckOne (ChuckOne@mail.de).
*/
package fruits.kit.crypto.rs;
...
public final class ReedSolomonImpl implements ReedSolomon {
  ...
  private static final int[] codeword_map = {3, 2, 1, 0, 7, 6, 5, 4, 13, 14, 15, 16, 12, 8, 9, 10, 11, 17, 19, 23, 18, 21, 20, 22};
  ...
  public String encode(long plain) {
    String plain_string = FruitsID.fromLong(plain).getID();
    ...
    for (int i = base_32_length - 1; i >= 0; i--) {
      final int fb = codeword[i] ^ p[10];
      p[10] = p[9] ^ this.gmult(9, fb);
      p[9] = p[8] ^ this.gmult(8, fb);
      p[8] = p[7] ^ this.gmult(7, fb);
      p[7] = p[6] ^ this.gmult(6, fb);
      p[6] = p[5] ^ this.gmult(5, fb);
      p[5] = p[4] ^ this.gmult(4, fb);
      p[4] = p[3] ^ this.gmult(3, fb);
