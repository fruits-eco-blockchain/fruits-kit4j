...
import fruits.kit.crypto.ec.Curve25519;
import fruits.kit.crypto.ec.Curve25519Impl;
import fruits.kit.crypto.ec.Curve25519NativeImpl;
import fruits.kit.crypto.hash.FruitsHashProvider;
...
import fruits.kit.crypto.rs.ReedSolomon;
import fruits.kit.crypto.rs.ReedSolomonImpl;
...
class FruitsCryptoImpl extends AbstractFruitsCrypto {

    static final FruitsCryptoImpl INSTANCE = new FruitsCryptoImpl();
    ...
    @Override
    public FruitsAddress getFruitsAddressFromPublic(byte[] publicKey) {
        FruitsAddress address = FruitsAddress.fromId(hashToId(getSha256().digest(publicKey)));
        address.setPublicKey(publicKey);
        return address;
    }

    @Override
    public FruitsID hashToId(byte[] hash) {
        if (hash == null || hash.length < 8) {
            throw new IllegalArgumentException("Invalid hash: " + Arrays.toString(hash));
        }
        return FruitsID.fromLong(bytesToLongLE(hash));
    }
    ...
    @Override
    public FruitsEncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(message, false, myPrivateKey, theirPublicKey);
    }

    @Override
    public FruitsEncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(stringToBytes(message), true, myPrivateKey, theirPublicKey);
    }

    private FruitsEncryptedMessage encryptPlainMessage(byte[] message, boolean isText, byte[] myPrivateKey, byte[] theirPublicKey) {
        if (message.length == 0) {
            return new FruitsEncryptedMessage(new byte[0], new byte[0], isText);
        }
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            return new FruitsEncryptedMessage(data, nonce, isText);
        } catch (IOException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    }
    ...
    @Override
    public byte[] decryptMessage(FruitsEncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey) {
    ...
    @Override
    public String rsEncode(FruitsID fruitsID) {
        return reedSolomon.encode(fruitsID.getSignedLongId());
    }

    @Override
    public FruitsID rsDecode(String rs) {
        rs = rs.toUpperCase();
        long rsValue;
        try {
        ...
        return FruitsID.fromLong(rsValue);
    }

    @Override
    public long fromFruitsTime(int fruitsTime) {
        return epochBeginning + (fruitsTime * 1000L);
    }

    @Override
    public int toFruitsTime(long unixTime) {
    ...
    @Override
    public byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, short dPages, short csPages, short usPages, FruitsValue minActivationAmount) {
