...
/**
 * The Interface FruitsCrypto.
 */
public interface FruitsCrypto {
    ...
    /**
     * Derive the fruits address of the passphrase.
     *
     * @param passphrase The passphrase
     * @return The fruits address
     */
    FruitsAddress getFruitsAddressFromPassphrase(String passphrase);

    /**
     * Derive the fruits address of the private key.
     *
     * @param privateKey The private key
     * @return The fruits address
     */
    FruitsAddress getFruitsAddressFromPrivate(byte[] privateKey);

    /**
     * Derive the fruits address of the public key.
     *
     * @param publicKey The public key
     * @return The fruits address
     */
    FruitsAddress getFruitsAddressFromPublic(byte[] publicKey);

    /**
     * Convert a full hash to an ID.
     *
     * @param hash The hash of an account, block, transaction etc
     * @return The Fruits ID of the account, block, transaction etc
     * @throws IllegalArgumentException if the hash is invalid
     */
    FruitsID hashToId(byte[] hash) throws IllegalArgumentException;
     ...
     * @return The encrypted message
     */
    FruitsEncryptedMessage encryptBytesMessage(byte[] message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    FruitsEncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    FruitsEncryptedMessage encryptTextMessage(String message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    FruitsEncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(FruitsEncryptedMessage message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(FruitsEncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey);

    /**
     * Get the Reed-Solomon encoding of a Fruits address. Does not include the "FRUITS-" prefix.
     * @param fruitsID The Fruits ID
     * @return The Reed-Solomon encoding of the address, not including the "FRUITS-" prefix.
     */
    String rsEncode(FruitsID fruitsID);

    /**
     * Get the account ID associated with a Reed-Solomon encoded address.
     *
     * @param rs The Reed-Solomon encoded address, not including the "FRUITS-" prefix
     * @return The ID of that RS address
     * @throws IllegalArgumentException If the RS address was invalid and could not be decoded.
     */
    FruitsID rsDecode(String rs) throws IllegalArgumentException;

    /**
     * Translate a number of seconds since the Fruits Epoch to a Java Date object.
     *
     * @param fruitsTime The number of seconds since the Fruits Epoch
     * @return This time represented as a Java Date object
     */
    Date fromFruitsTimeToDate(int fruitsTime);

    /**
     * Translate a number of seconds since the Fruits Epoch to a Unix time (number of milliseconds since Unix Epoch).
     *
     * @param fruitsTime The number of seconds since the Fruits Epoch
     * @return This time represented as a Unix time (number of milliseconds since Unix Epoch)
     */
    long fromFruitsTime(int fruitsTime);

    /**
     * Converts a Date to the number of seconds since the Fruits Epoch.
     *
     * @param date The target time
     * @return The number of seconds from the Fruits Epoch to this time.
     */
    int toFruitsTime(Date date);

    /**
     * Converts a number of milliseconds since the Unix epoch
     * to the number of seconds since the Fruits Epoch.
     *
     * @param unixTime The target time, as a unix epoch - for example from System.currentTimeMillis()
     * @return The number of seconds from the Fruits Epoch to this time.
     */
    int toFruitsTime(long unixTime);

    /**
     * Gets the current Fruits Time - the number of seconds since the Fruits Epoch.
     * @return The number of seconds since the Fruits Epoch
     */
    int currentFruitsTime();
     ...
     * @return The generation signature of the block
     */
    byte[] calculateGenerationSignature(byte[] lastGenSig, FruitsAddress lastGenerator);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateHit(FruitsAddress accountId, long nonce, byte[] genSig, int scoop, int pocVersion);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateDeadline(FruitsAddress accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int pocVersion);

    /**
     * TODO javadoc.
     *
     * @param atVersion the at version
     * @param code the code
     * @param data the data
     * @param dPages the d pages
     * @param csPages the cs pages
     * @param usPages the us pages
     * @param minActivationAmount the min activation amount
     * @return the AT creation bytes
     */
    byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, short dPages, short csPages, short usPages, FruitsValue minActivationAmount);
    ...
    /**
     * Sets the native enabled.
     *
     * @param enabled the new native enabled
     */
     ...
     */
    static FruitsCrypto getInstance() {
        return FruitsCryptoImpl.INSTANCE;
    }
}
