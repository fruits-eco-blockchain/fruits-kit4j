...
public class TransactionAttachmentAndAppendagesResponse {
    ...
    private static TransactionAttachmentAndAppendagesResponse deserialize(JsonObject source) {
        Gson gson = FruitsKitUtils.buildGson().create();
        ...
        } else if (source.has( "version.AssetIssuanceWithIcon")) {
            attachment = gson.fromJson(source, AssetIssuanceWithIconAttachmentResponse.class);
        } else if (source.has( "version.AssetAdditionalIssuance")) {
            attachment = gson.fromJson(source, AssetAdditionalIssuanceAttachmentResponse.class);
    ...
    private static JsonElement serialize(TransactionAttachmentAndAppendagesResponse source) {
        return FruitsKitUtils.buildGson().create().toJsonTree(source); // TODO this won't work...
    }
}
